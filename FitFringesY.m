...
%   Detailed explanation goes here
   ...
   methods
       function appData = analyze(obj, appData) % do the analysis
           ...
           firstGuess = [fitObj.OD*1, fitObj.C,  10, 0, 0.1, fitObj.sigmaY, fitObj.y0-y0];
...
%                 'dependent',{'y'},'independent',{'x'}, 'coefficients',{'a', 'c', 'lambda', 'phi', 'v', 'w', 'x0'}); %relative phase
            ft = fittype('a*exp(-(x-x0)^2/2/w^2)*(1+v*sin(2*pi/lambda*(x-length(x)/2)+phi))+c',...
                'dependent',{'y'},'independent',{'x'}, 'coefficients',{'a', 'c', 'lambda', 'phi', 'v', 'w', 'x0'});%absolute phase
            obj.a = fitObj.OD;
            obj.x0 = fitObj.y0-y0;
            obj.w = fitObj.sigmaY;            
            obj.c = fitObj.C;
...
%                (1+obj.res.v*sin(2*pi*(y-obj.x0)/obj.res.lambda+obj.res.phi))+obj.c;
       ...
       function  plotFitResults(obj, appData)  % plots the text
           ...
           text(10, 150, ['visibility = ' num2str(obj.res.v) ' +/- ' num2str(obj.conf(5) )]);
